<!-- Page header -->
									...
									if (!empty($dKualitas)){ 
										...
										?>
											<tr>
												<td></td>
												<td><?= $no ?></td>
												<td><?= $data->KualitasAir ?></td>
												<td><?= date_format(date_create($data->TglBuat), 'd F Y') ?></td>
												<td><?= $data->NamaPetugas ?></td>
												<td class="text-center">
													<i class="icon-pencil text-primary" data-popup="tooltip" title="Rubah" style="cursor:pointer" onclick="modalRubah('<?= $data->IdKualitasAir ?>');" ></i>
													<i class="icon-trash text-danger-600" data-popup="tooltip" title="Hapus" style="cursor:pointer" onclick="konfirmasi(<?= "'Hapus ?','Parameter ".$data->KualitasAir."','".$data->IdKualitasAir."','".base_url('kualitasair/hapus')."'"; ?>);" ></i>
												</td>
											</tr>
								<?php
											$no++;
				...
				<script type="text/javascript">
					function modalTambah(){
	                	$.post('<?= base_url('kualitasair/tambah'); ?>', {} , function(mod) {
	                ...
	                function modalRubah(index){
	                	$.post('<?= base_url('kualitasair/rubah'); ?>', {id:index} , function(mod) {
	                		$('#tampilModal').html(mod);
	                		$('#modalForm').modal({show: true , backdrop : true , keyboard: true});
	                	});
	                }

	                $(document).on("click", "#btnKualitas", function () {
	                	$('#modalKualitas').validate({
				        ...
				        submitHandler: function () {
				        	$.post('<?= base_url('kualitasair/simpan') ?>', $('#modalKualitas').serialize(), function(resp){
				        		if (resp == 'sama'){
				        			$("#textModalError").text("Maaf, Nama penilaian kualitas ir yang anda input sudah terdaftar !");
