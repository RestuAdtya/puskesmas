<!-- Page header -->
													...
													<ul class="icons-list">
														<li class="dropdown">
															...
															<ul class="dropdown-menu dropdown-menu-right">
																<li><a href="#"  onclick="modalRubah('<?= $data->IdKategoriTpm ?>');" ><i class="icon-pencil text-primary-600"></i> Rubah data</a></li>
																<li><a href="#" onclick="konfirmasi(<?= "'Hapus ?','Kategori Tempat-tempat umum ".$data->KategoriTpm."','".$data->IdKategoriTpm."','".base_url('kategoritpm/hapus')."'"; ?>);"><i class="icon-trash text-danger-600"  ></i> Hapus data</a></li>
															</ul>
														</li>
													</ul>
												</td>
				...
				<script type="text/javascript">
					function modalTambah(){
	                	$.post('<?= base_url('kategoritpm/tambah'); ?>', {} , function(mod) {
	                ...
	                function modalRubah(index){
	                	$.post('<?= base_url('kategoritpm/rubah'); ?>', {id:index} , function(mod) {
	                		$('#tampilModal').html(mod);
	                		$('#modalForm').modal({show: true , backdrop : true , keyboard: true});
	                	});
	                }
	                ...
	                $(document).on("click", "#btnKategori", function () {
				        ...
				        submitHandler: function () {
				        	$.post('<?= base_url('kategoritpm/simpan') ?>', $('#modalKategori').serialize(), function(resp){
				        		if (resp == 'sama'){
				        			$("#textModalError").text("Maaf, Nama kategori tempat pengelolaan makanan yang anda input sudah terdaftar !");
